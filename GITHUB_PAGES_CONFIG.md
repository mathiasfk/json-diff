...
## Solução
Mudar a source do Pages para **branch `gh-pages`**. Isso permite que tanto o deploy de produção (`deploy.yml`) quanto o preview (`preview.yml`) funcionem no mesmo mecanismo.
...
| Workflow | O que faz |
|---|---|
| `deploy.yml` | Deploy de produção → `gh-pages/` (raiz) |
...
2. O `deploy.yml` precisa ser revertido para usar `actions/configure-pages` + `upload-pages-artifact` + `deploy-pages`
...
- Após a mudança, o primeiro deploy via `deploy.yml` pode levar ~1-2 minutos para propagar
